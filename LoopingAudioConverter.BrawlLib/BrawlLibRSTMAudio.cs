...
namespace LoopingAudioConverter.BrawlLib {
    public unsafe class BrawlLibRSTMAudio : PCM16Audio, IDisposable {
        ...
        private BrawlLibRSTMAudio(PCM16Audio audio, IntPtr address, RSTMNode node) : base(audio.ImmutableAudio, node.IsLooped ? Loop.NewLoop(node.LoopStartSample, node.NumSamples) : Loop.NonLooping) {
            _address = address;
            _node = node;
        }
        ...
        public byte[] ExportRSTM() {
            if (!OriginalLoop.Equals(CurrentLoop))
